...
public function addfound($d)
{
$string = array(
'name'=>$d['name'],
'place'=>$d['place'],
'place_detail'=>$d['place_detail'],
'item_details'=>$d['item_details'],
'contact'=>$d['contact'],
'pub_date'=>$d['pub_date'],
'pic_related'=>$d['pic_related'],
'user_id' => $d['user_id'],
);
$q = $this->db->insert_string('items',$string);
$this->db->query($q);
return $this->db->insert_id();
    ...
    public function getItemInfo($id)
    {
        ...
        if($this->db->affected_rows() > 0){
            ...
            error_log('no item found getItemInfo('.$id.')');
            return false;
        }
    }
    ...
    public function getItemAllData($id)
    {
        ...
        {
            error_log('no user found getItemAllData('.$id.')');
            return false;
        }
    }
      //update data user
      public function updatefound($post)
      {
          $data = array(
                 'name' => $post['name'],
                 'place' => $post['place'] ,
                 'place_detail' => $post['place_detail'],
                 'item_details' => $post['item_details'], 
                 'contact' => $post['contact'],
                 'pub_date' => $post['pub_date'],
                 'pic_related' => $post['pic_related'], 
                 //'status' => 'active',
    ...
    {   
    	...
    	$this->db->select('count(*) as allcount');
      	...
      	if($search != ''){
			...
			$this->db->or_like('pub_date', $search);
      	}
