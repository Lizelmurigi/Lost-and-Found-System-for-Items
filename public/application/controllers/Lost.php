...
class lost extends CI_Controller {
    ...
    function __construct(){
        ...
        $this->load->model('Lost_model', 'lost_model', TRUE);
    ...
    public function lists($rowno=0)
    {
		...
		$allcount = $this->lost_model->countItemData($search_text);
		//$users_record = 
		
		$data['groups'] = $this->lost_model->getItems($rowno,$rowperpage,$search_text);
		// Pagination Configuration
      	$config['base_url'] = base_url().'index.php/lost/lists';
   ...
   public function changeitem() 
   {
     $data = $this->session->userdata;
       ...
       //$data['classes']= $this->lost_model->clas();
       $data['groups'] = $this->lost_model->getItemInfo($this->uri->segment(3));

       if ($this->form_validation->run() == FALSE) {
           $this->load->view('header', $data);
           $this->load->view('navbar', $data);
           $this->load->view('container');
           $this->load->view('lost/edit', $data);
           $this->load->view('footer');
       }else{
...
if($dataLevel == "is_admin"){
    ...
    $this->lost_model->deleteItem($id);
    if($this->lost_model->deleteItem($id) == FALSE )
    {
...
public function add()
{
    ...
    if($dataLevel == "is_admin" || $dataLevel == "is_user"){
        ...
        $this->form_validation->set_rules('place_details', 'Place details', 'required');
        $this->form_validation->set_rules('item_details', 'Item description', 'required');
        $this->form_validation->set_rules('lost_date', 'Published Date', 'required');
       // $this->form_validation->set_rules('gender', 'Gender Confirmation', 'required');
        $data['classes'] = $this->lost_model->item();
        $data['title'] = "Add Lost items";
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('header', $data);
            $this->load->view('navbar');
            $this->load->view('container');
            $this->load->view('lost/add', $data);
            $this->load->view('footer');
        }else{
            if($this->lost_model->isDuplicate($this->input->post('email'))){
                $this->session->set_flashdata('flash_message', 'User email already exists');
                redirect(site_url().'lost/add');
            }else{
                ...
                //insert to database
                if(!$this->lost_model->addlost($cleanPost)){
